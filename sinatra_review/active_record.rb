...
class ActiveFaker::Migration
    ...
    def create_table(table_name) #create_table :users
      #CREATE TABLE(Table_name) id Integer type
        query = QueryManager.new("CREATE TABLE #{table_name} (id INTEGER PRIMARY KEY")
        yield(query) #the t value is the query
        query.complete_query
        puts query.current_query
        #if connected to a database you would put:
        #DB[:conn].execute(query.current_query)
    end

    def add_column(table_name, col_name, col_type)
       #In sQL you would add a column by doing the collofing
    ...
    class QueryManager
        ...
        def string(col_name) #t.string
           # @current_query += ", #{col_name}, TEXT" -- (WITHOUT ABSTRACTION)
            add_to_query(col_name, "TEXT")
        end

        def integer(col_name) #t.intenger
            # @current_query += ", #{col_name}, INTEGER" -- (WITHOUT ABSTRACTION)
        ...
        def add_to_query(col_name, type) #abstraction
...
class ActiveFaker::Base #Makes the belongs to work
